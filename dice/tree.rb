...
class SimpleTree
  ...
  bloom do
    ins_state <= (ins_init * root).pairs {|i,r| [i.op_id, r.node_id, 0]}
    ...
    max_depth <= ins_state.group([:op_id], max(:depth))
    edge <+ (max_depth * ins_state).rights(:op_id => :op_id, :depth => :depth) do |i|
